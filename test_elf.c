...
int main(int argc, char **argv)
{
    ...
    if (elf_version(EV_CURRENT) == EV_NONE) errx(EXIT_FAILURE, "ELF library init failed: %s", elf_errmsg(-1));
    if ( (fd = open(PROGRAM_NAME, O_RDONLY, 0)) < 0) err(EXIT_FAILURE, "open failed");
    ...
    if (elf_getphdrnum(e, &n) != 0) errx(EXIT_FAILURE, "elf_getphdrnum() failed %s", elf_errmsg(-1));

    for (int i = 0; i < n; i++) {

        if (gelf_getphdr (e , i, &phdr) != & phdr) errx(EXIT_FAILURE, "getphdr() failed %s", elf_errmsg(-1));
        printf("PHDR %d : \n", i);
