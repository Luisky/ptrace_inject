...
void hotwait(pid_t pid)
{
    ...
    if (WIFSTOPPED(wstatus))
        printf("process %d was stopped by signal number %d\n", pid, WSTOPSIG(wstatus));
...
void init_ptrace_attach(pid_t pid)
{
    if (ptrace(PTRACE_ATTACH, pid, NULL, NULL) == -1)
        err(EXIT_FAILURE, "Ptrace_failed");
    hotwait(pid);
...
void write_trap_only(pid_t pid, Arg *arg)
{
    ...
    if (lseek(fd, (off_t)arg->func_addr, SEEK_SET) < 0)
        perror("lseek");
    if (read(fd, &(arg->old_byte), 1) < 0)
        perror("read");

    if (lseek(fd, (off_t)arg->func_addr, SEEK_SET) < 0)
        perror("lseek");
    if ((br = write(fd, &int3, 1)) < 0)
        perror("write");
    ...
    while (getline(&line, &len, fp_maps) != -1)
    {
        ...
        if (path_or_desc[0] == '[')
        {
            ...
            if (strcmp(str, STACK) == 0)
                return addr_start;
            if (strcmp(str, VVAR) == 0)
                return addr_start;
            if (strcmp(str, VDSO) == 0)
                return addr_start;
            if (strcmp(str, VSYSCALL) == 0)
                return addr_start;
        }
...
uint64_t get_symbol_offset(char *exec_path, char *sym_name, bool dynamic)
{
    ...
    if (dynamic)
        sprintf(exec, "/usr/bin/nm -D %s | grep %s | head -n 1 |cut -f 1 -d \" \"", exec_path, sym_name);
    else
        sprintf(exec, "/usr/bin/nm %s | grep %s | head -n 1 |cut -f 1 -d \" \"", exec_path, sym_name);
